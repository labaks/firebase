...
abstract class FirebaseCustomAdapter extends FirebaseListAdapter<Item> implements Filterable {
...
//                query = FirebaseDatabase.getInstance().getReference().child("items").orderByChild("name").equalTo(constraint.toString());
                query = FirebaseDatabase.getInstance().getReference().child("items").orderByChild("name").startAt(constraint.toString());
                Log.e("Filtering1", query.toString());
            } else {
                query = FirebaseDatabase.getInstance().getReference().child("items").orderByChild("name").startAt(constraint.toString());
                Log.e("Filtering2", query.toString());
            }
